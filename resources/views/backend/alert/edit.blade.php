...
@section('title') {{'Edit Alert'}} @endsection

@section('breadcrumb') Alert / Edit Alert @endsection
@section('page-title') Edit Alert @endsection
                        ...
                        <form action="{{ route('alert.update', $alert->id) }}" method="POST">
                            ...
                            <div class="mb-20">
                                ...
                                <select class="form-control radius-8 form-select @error('system_id') is-invalid @enderror" id="system_id" name="system_id">
                                    ...
                                    @foreach($systems as $system)
                                        <option value="{{ $system->id }}" {{ (old('system_id', $alert->system_id) == $system->id) ? 'selected' : '' }}>
                            ...
                            <div class="mb-20">
                                ...
                                <select class="form-control radius-8 form-select @error('severity') is-invalid @enderror" id="severity" name="severity">
                                    ...
                                    <option value="warning" {{ (old('severity', $alert->severity) == 'warning') ? 'selected' : '' }}>Warning</option>
                                    <option value="info" {{ (old('severity', $alert->severity) == 'info') ? 'selected' : '' }}>Info</option>
                                </select>
                                @error('severity')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-20">
                                ...
                                <input type="text" name="alert_type" id="alert_type" class="form-control @error('alert_type') is-invalid @enderror" value="{{ old('alert_type', $alert->alert_type) }}" placeholder="e.g., Inverter Offline, Low Production">
                                @error('alert_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-20">
                                ...
                                <textarea name="message" id="message" class="form-control @error('message') is-invalid @enderror" rows="4" placeholder="Detailed description of the alert...">{{ old('message', $alert->message) }}</textarea>
                                @error('message')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-20">
                                ...
                                <select class="form-control radius-8 form-select @error('status') is-invalid @enderror" id="status" name="status">
                                    ...
                                    <option value="scheduled" {{ (old('status', $alert->status) == 'scheduled') ? 'selected' : '' }}>Scheduled</option>
                                    <option value="in_progress" {{ (old('status', $alert->status) == 'in_progress') ? 'selected' : '' }}>In Progress</option>
                            ...
                            <div class="mb-20">
                                ...
                                <input type="datetime-local" name="resolved_at" id="resolved_at" class="form-control @error('resolved_at') is-invalid @enderror" value="{{ old('resolved_at', $alert->resolved_at ? $alert->resolved_at->format('Y-m-d\TH:i') : '') }}">
                                @error('resolved_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
